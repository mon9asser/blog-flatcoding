...
export default async function handler(req, res) {
  try {
    ...
    if (!name || !email || !interaction_type || !comment_id) {
      return res.send({
        is_error: true,
        message: 'Missing required fields: name, email, comment_id, or interaction_type.',
        data: []
      });
    }
    ...
    const wp_response = await Helper.sendWPRequest({
      api: `wp-json/custom/v1/comment-interaction`,
      method: "post",
      data: data_form
    });
