...
export default function Tag({upcoming}) {
    ...
    var load_more_posts = async (e) => {
        ...
        */ 
        ...
        var latest_request = await Helper.sendNTRequest({
            api: `latest_posts?page_number=${current_page}&tag=${upcoming.tag_data.name}`,
            method: "get",
            data: {}
        });
    ...
    var json_data_var = `{
        ...
        "mainEntityOfPage": {
            "@type": "WebPage",
            "@id": "${upcoming?.tag_data?.link}"   
        },
        "url": "${upcoming?.tag_data?.link}",
    ...
    return <>
        <Head>
            ...
            <link rel="canonical" href={upcoming?.tag_data?.link}/>
            ...
            <meta property="og:url" content={upcoming?.tag_data?.link}/>
...
export async function getServerSideProps(context) {
    ...
    try {
        ...
        const requests = [
            ...
            Helper.sendWPRequest({
                api: `wp-json/custom/v1/latest-posts?tag=${tag}&page_number=1`,
                method: "get",
                data: {}
            }),
            Helper.sendWPRequest({
            ...
            Helper.sendWPRequest({
                api: `wp-json/wp/v2/tags?slug=${tag}`,
                method: "get",
                data: {}
            }),
        ];
        ...
        tag_data = tag_data[tag_data.length - 1];
        tag_data.link = tag_data.link.replace("authors.flatcoding.com", "flatcoding.com/blog");
