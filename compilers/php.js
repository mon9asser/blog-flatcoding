...
var phpCompiler = (req, res) => {
    try {
        ...
        fs.writeFile(filepath, code, (err) => {
            ...
            var execution = spawn( executions.php, [filepath], {
                cwd: tempDir,
                timeout: 1000,
            ...
            execution.on('error', (err) => {
                if (err.code === 'ENOENT') {
                    return res.send({
                        is_error: true, 
                        message: 'PHP executable not found. Please make sure PHP is installed and in your PATH.',
                        data: ''
                    })
                } else {
                    return res.send({
                        is_error: true, 
                        message: 'An error occurred while trying to spawn the PHP process:', err,
                        data: ''
                    }) 
                }
            });
